        ...
        <div class="col-md-6 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h3>Добавить запись антифриза</h3>
                </div>
                <div class="card-body">
                    ...
                    <?php endif; ?>
                    <form action="/military-antifreeze/records/store" method="POST">
                        <div class="mb-3">
                            <label for="date" class="form-label">Дата *</label>
                            <input type="date" class="form-control" id="date" name="date"
                                   value="<?= $_SESSION['old']['date'] ?? date('Y-m-d') ?>" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="mt_antifreeze_id" name="mt_antifreeze_id" required>
                                ...
                                <?php foreach ($MilitaryAntifreeze as $a): ?>
                                    <option value="<?= $a['id'] ?>" <?= ($_SESSION['old']['mt_antifreeze_id'] ?? '') == $a['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="machine_id" name="machine_id" required>
                                ...
                                <?php foreach ($MilitaryModelMachine as $m): ?>
                                    <option value="<?= $m['id'] ?>" <?= ($_SESSION['old']['machine_id'] ?? '') == $m['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="value" name="value" min="1"
                                   value="<?= $_SESSION['old']['value'] ?? '' ?>" required>
                        </div>
